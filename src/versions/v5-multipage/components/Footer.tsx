...
import { FOOTER_LEGAL_ITEMS, NAV_ITEMS, pages } from "../content/nav";
...
function FooterNavLink({ path, label }: { path: string; label: string }) {
  const to = useVersionPath(path);
  return (
    <Link
      to={to}
      className="pnw-text-link font-normal underline hover:opacity-70 transition-opacity"
    >
      {label}
    </Link>
  );
...
export function Footer() {
  ...
  const siteLinks = NAV_ITEMS.filter((item) => item.id !== "contact");
      ...
      <div className="pnw-container flex flex-col md:flex-row md:items-start w-full gap-8 md:gap-4 text-left text-sm">
        ...
        <div className="flex flex-col gap-1 md:flex-1">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="pnw-text-link inline-flex items-center gap-2 hover:opacity-70 transition-opacity w-fit"
        ...
        <div className="flex flex-col gap-1 md:flex-1">
          ...
          {siteLinks.map((item) => (
            <FooterNavLink key={item.id} path={item.path} label={item.label} />
          ))}

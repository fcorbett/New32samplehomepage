...
function Stars() {
  return (
    <div className="flex gap-[2px]">
      {[...Array(5)].map((_, i) => (
        <svg key={i} viewBox="0 0 24 24" width="14" height="14" fill="var(--pnwb-sage)">
...
function RatingCard() {
  ...
  return (
    <a
      ...
      className="flex flex-col gap-3 p-6 border-l-2 border-[var(--pnwb-sage)] bg-[var(--pnwb-white)] hover:bg-[var(--pnwb-stone-deep)] transition-colors w-full group"
    >
      <div className="flex items-center gap-2 opacity-90 group-hover:opacity-100 transition-opacity">
        <GoogleIcon />
        <span className="text-sm text-[var(--pnwb-ink-soft)] font-medium">
          Google reviews
        </span>
      </div>
      <div className="flex items-end gap-3">
        <span className="text-3xl font-display text-[var(--pnwb-ink)] leading-none">
      ...
      <span className="text-xs text-[var(--pnwb-ink-soft)]/70 tracking-wide">
...
export function Intro() {
  return (
    <section id="about" className="w-full px-6 md:px-16 py-16 md:py-24 lg:py-28">
      <div className="max-w-4xl mx-auto">
        <ScrollReveal>
          <div className="grid grid-cols-1 md:grid-cols-[1fr_280px] gap-10 md:gap-14 items-start">
            <div>
              <p className="font-semibold text-[var(--pnwb-sage)] text-sm tracking-wide mb-6">
                Welcome to new32
              </p>
              <div className="font-display font-normal text-[var(--pnwb-ink)] text-xl md:text-2xl leading-[1.5] space-y-5 max-w-[58ch]">
                <p>
